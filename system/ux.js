...
function swiper() {
    ...
    $('.swiper-pagination').html('<span class="swiper-pagination-current">E</span><div></div><span class="swiper-pagination-total">P</span>');
}
$('.slide-num').attr('data-swiper-parallax-y', '-175%');
...
function nav() {$('.nav').css('left', nav_left); $('header li:first-of-type').addClass('active').attr('onclick', 'nav_c();');}
function nav_c() {$('.nav').css('left', '-125%'); $('header li:first-of-type').removeClass('active').attr('onclick', 'nav();');}
...
$('a, button, input, .snipcart-add-item, header li, .swiper-button-next, .swiper-button-prev').hover(function(){$('.cursor, .circle').addClass('hover');}, function(){$('.cursor, .circle').removeClass('hover');});
...
function scroll() {
	...
	$('main').animate({scrollTop: $('aside').offset().top}, scroll_time); toggleFullScreen(elem);}
...
function scroll_p() {
	...
	$('main').queue([]).stop(); toggleFullScreen(elem);}
